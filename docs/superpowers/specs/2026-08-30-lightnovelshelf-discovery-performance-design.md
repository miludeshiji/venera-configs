...
现有 `_hubCall(target, params)` 改为通过 `_runHubSession` 执行单次 `_hubInvoke`，保持所有普通请求的既有行为。章节 `loadEp` 也通过 `_runHubSession` 执行完整章节操作，使账号切换失效并让一次 unauthorized 重试重新创建操作局部状态。发现页则在一次 `_runHubSession` 中完成两个依赖阶段。

操作回调可能因 unauthorized 执行两次，因此发现页和章节加载回调内部只使用局部变量，不提前更新实例状态。只有整套调用成功并通过请求代际检查后才提交状态。

每个 Hub session 在创建时固定记录认证代际、RefreshToken 和会话 Token。negotiate、handshake、invoke、poll 与 close 均使用该 session 的 Token，而不是读取可能已被并发请求替换的全局 Token。收到 unauthorized 时，仅当全局认证代际、RefreshToken 和 Token 仍与失败 session 的所有权完全匹配，才清除全局短期 Token。
...
任一实际 Hub 调用失败时，整个发现页仍按现有行为加载失败，不把认证或服务故障伪装为空区块；仅请求代际失效且没有同账号替代请求时，按上文约定返回空的中性区块。
...
Venera 当前会把漫画源 Promise 的拒绝转换成 `Res.error`，发现页刷新状态又不会携带请求代际，因此源侧对**没有同账号替代请求**的失效发现请求返回三个空的中性区块，而不是继续抛出“请求已失效”错误。这样可避免旧错误覆盖刷新后的页面；中性结果不提交历史状态，也不代表服务端请求成功。若存在同账号替代请求，旧 Promise 复用替代 Promise 的结果。
...
6. 退出、刷新和并发请求后旧结果不能污染当前历史状态；无同账号替代请求时，失效发现 Promise 返回三个空的中性区块而不抛出错误；
