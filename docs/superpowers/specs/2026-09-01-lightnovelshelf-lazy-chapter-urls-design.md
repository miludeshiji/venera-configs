...
2. 打开章节时至多探测第一个 6 页批次，以取得服务端报告的准确总页数；
...
首批保存于状态中，但返回数组仍统一使用虚拟键。这样所有页面走相同解析路径；第 1～6 页的 `onImageLoad` 只读取已完成首批 Promise，不会再次请求 Hub。
...
预期不会请求 `Skip: 6` 到 `Skip: 108`。Venera 默认预加载末页附近图片时，它们都落在 `Skip: 114` 的同一批次，因此只复用该批次。
...
1. 13 页章节的 `loadEp` 只请求 `Skip: 0`，返回 13 个唯一虚拟键；
2. 加载第 13 页后只新增 `Skip: 12`，不请求 `Skip: 6`；
